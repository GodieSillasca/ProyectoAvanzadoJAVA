		...
		while(true){
			...
			for (int i = 0;i<8 ;i++ ) {
				...
				if(recibido.lin1.get(i)==true){
					Runnable t = new Reproducir("./1tarola.wav");
					Thread c1 = new Thread(t);
					c1.start();
				}

				if(recibido.lin2.get(i)==true){
					Runnable t = new Reproducir("./2percusion.wav");
					Thread c1 = new Thread(t);
					c1.start();
				}

				if(recibido.lin3.get(i)==true){
					Runnable t = new Reproducir("./3bombo.wav");
					Thread c1 = new Thread(t);
					c1.start();
				}

				if(recibido.lin4.get(i)==true){
					Runnable t = new Reproducir("./4hi_hat.wav");
					Thread c1 = new Thread(t);
					c1.start();
				}
